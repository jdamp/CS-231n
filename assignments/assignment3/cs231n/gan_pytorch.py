...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    labels_real = torch.ones_like(logits_real).type(dtype).squeeze()
    labels_fake = torch.zeros_like(logits_fake).type(dtype).squeeze()
...
def generator_loss(logits_fake):
    ...
    true_labels = torch.ones_like(logits_fake).type(dtype).squeeze()
